...
export default function Navbar() {
  ...
  // Updated nav items with conditional "Dashboard" and "My Orders" links
  const getNavItems = () => {
    const baseItems = [...navItems];

    // Add "Dashboard" after Shop link for logged-in users
    if (isLoggedIn) {
      ...
      if (shopIndex !== -1) {
        ...
        baseItems.splice(shopIndex + 2, 0, {
          name: "My Orders",
          path: "/myorders",
          icon: <FiPackage />
        });
      ...
      <div className={navbarStyles.container}>
        <div className={navbarStyles.innerContainer}>
          ...
          <Link to="/" className={navbarStyles.logoLink}>
            <img
              src={logo}
              alt="𝔸ℕ𝕊ℍ𝕌 𝔹𝔼𝔸𝕌𝕋𝕐 ℍ𝕌𝔹 Logo"
              className={`${navbarStyles.logoImage} ${scrolled ? 'h-9 w-9' : 'h-11 w-11'}`}
            />
            <span className={navbarStyles.logoText}>𝔸ℕ𝕊ℍ𝕌 𝔹𝔼𝔸𝕌𝕋𝕐 ℍ𝕌𝔹 </span>
          </Link>
        ...
        >
            <div className={navbarStyles.mobileHeader}>
              <div className={navbarStyles.mobileLogo}>
                <img
                  src={logo}
                  alt="ANSHU BEAUTY HUB Logo"
                  className={navbarStyles.mobileLogoImage}
                />
                <span className={navbarStyles.mobileLogoText}>ANSHU BEAUTY HUB</span>
              </div>
              <button
                onClick={() => setIsOpen(false)}
