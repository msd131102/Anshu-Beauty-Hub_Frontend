...
const OrdersPage = () => {
  ...
  const fetchOrders = async () => {
    try {
      const { data } = await axios.get('https://anshu-beauty-hub-backend.onrender.com/api/orders');
      setOrders(data);
  ...
  const updateOrderStatus = async (orderId, newStatus) => {
    try {
      await axios.put(
        `https://anshu-beauty-hub-backend.onrender.com/api/orders/${orderId}`,
        { status: newStatus }
      );
      setOrders(prev =>
        prev.map(order =>
                    ...
                    <div className={styles.modalOrderSummary}>
                      {selectedOrder.items.map((item, index) => (
                        ...
                        >
                          {item.imageUrl ? (
                            <img
                              src={`https://anshu-beauty-hub-backend.onrender.com${item.imageUrl}`}
                              alt={item.name}
