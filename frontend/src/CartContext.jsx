...
export const CartProvider = ({ children }) => {
  ...
  const fetchCart = async () => {
    try {
      const { data } = await axios.get(
        'https://anshu-beauty-hub-backend.onrender.com/api/cart',
        {
          ...getAuthHeader(),
  ...
  const refreshCart = async () => {
    try {
      const { data } = await axios.get('https://anshu-beauty-hub-backend.onrender.com/api/cart', getAuthHeader());
      const rawItems = Array.isArray(data)
        ? data
        : Array.isArray(data.items)
          ? data.items
  ...
  const updateQuantity = async (lineId, quantity) => {
    try {
      await axios.put(
        `https://anshu-beauty-hub-backend.onrender.com/api/cart/${lineId}`,
        { quantity },
        getAuthHeader()
      );
      await refreshCart();
  ...
  const removeFromCart = async (lineId) => {
    try {
      await axios.delete(`https://anshu-beauty-hub-backend.onrender.com/api/cart/${lineId}`, getAuthHeader());
      await refreshCart();
    } catch (err) {
  ...
  const clearCart = async () => {
    try {
      await axios.post('https://anshu-beauty-hub-backend.onrender.com/api/cart/clear', {}, getAuthHeader());
      setCart([]);
    } catch (err) {
